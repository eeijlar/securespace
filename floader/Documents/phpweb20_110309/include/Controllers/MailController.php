<?<?php
    class MailController extends CustomControllerAction {
		...
		function decrypt($encrypted_text,$key,$iv,$bit_check){
			$ci<?phper = mcrypt_module_open(MCRYPT_TRIPLEDES,'','cbc','');
			mcrypt_generic_init($ci<?phper, $key, $iv);
			$decrypted = mdecrypt_generic($ci<?phper,base64_decode($encrypted_text));
			mcrypt_generic_deinit($ci<?phper);
