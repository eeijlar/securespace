    ...
    class DatabaseObject_User extends DatabaseObject
    {
        ...
        protected function postInsert()
        {
            ...
            if($this->user_type == 'member'){ 
                ...
                $this->profile->setUserId($session->sme_id);
                $this->profile->load();
                $this->sendEmail('sme-register.tpl');
	     }
        ...
        public function createAuthIdentity()
        {
            ...
            $identity->clients = null; 
        
            	if ($identity->user_type == 'administrator' || $identity->user_type == 'sme' || $identity->user_type == 'superuser'){
        ...
        public function fetchIdFromFullName($fullname){
                $extractedString = explode(" ",$fullname);
                $name = "'first_name', 'last_name'";
                        $query = sprintf("select distinct user_id from %s where profile_key in (%s) and profile_value in ('%s','%s')",
                                                        "users_profile",
                                                        $name,
                                                        $extractedString[0],
                                                        $extractedString[1]);
