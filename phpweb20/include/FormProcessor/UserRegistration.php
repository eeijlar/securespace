<?php
    class FormProcessor_UserRegistration extends FormProcessor
    {
        protected $db = null;
        public $user = null;
        public $status = 'Application Complete';
        ...
        public function process(Zend_Controller_Request_Abstract $request)
        {
            ...
            $this->user->profile->story = $this->encrypt($this->sanitize($strStory),$this->encKey,$this->iv,$this->bit_check); 
			
			if (strlen($this->user->profile->story) < 50){
				$this->addError('story', 'Please enter your story. It must be at least 50 characters long.');			
			}
            ...
            if (!$this->_validateOnly && !$this->hasError()) {
                ...
                $this->user->profile->date = date("F j, Y, g:i a"); 
                $this->user->save();
