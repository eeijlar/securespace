    ...
    class AccountController extends CustomControllerAction
    {
            ...
            if ($request->isPost()) {
            		if ($validate){
	                    ...
	                    $session->user_id = $fp->user->getId();
                            $mailProcessor = new FormProcessor_NewMessage($this->db,$session->user_id);
                            $smeSession = new Zend_Session_Namespace('sme');
                            if($mailProcessor->updateClientSentItems($session->user_id, $smeSession->sme_id)){                     
	                        $this->_redirect('/account/registercomplete');
                            } else {
                               $this->_redirect('/account/registerfailed'); 
                            }
	                }          	
            } 
        ...
        public function registercompleteAction()
        {
            ...
            if (!$user->load($session->user_id)) {
                ...
                $inboxProcessor = new FormProcessor_NewMessage($this->db,$session->user_id);
                $smeSession = new Zend_Session_Namespace('sme');
                if (!$inboxProcessor->updateClientInbox($session->user_id,$smeSession->sme_id,'housekeeping.tpl')){
                  $this->_redirect('/account/registerfailed');
                }
            }
        ...
        public function logoutAction()
        {
            ...
            $this->_redirect('http://www.securecounselling.ie/index');
        }
